      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            <h3 className="text-base font-bold text-gray-900 truncate leading-tight group-hover:text-indigo-600 transition-colors">
        ...
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          <Chip
            size="small"
            label={status.label}
            className={`${status.bg} ${status.color} font-medium text-xs`}
          />
          <Chip
            size="small"
            label={application.platform}
            className={`${platformColor.bg} ${platformColor.text} font-medium capitalize text-xs`}
          />
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-gray-100">
          ...
          <Button
            size="small"
            variant="text"
            endIcon={<ArrowSquareOut weight="bold" size={12} />}
            onClick={() => window.open(application.jobUrl, "_blank")}
    ...
    <tr className="hover:bg-indigo-50/30 transition-colors group">
      <td className="px-4 py-3">
        <div className="flex flex-col">
          ...
          <span className="text-xs text-gray-500">{application.company}</span>
        </div>
      </td>
      <td className="px-4 py-3">
        <Chip
...
// Sidebar Stats Card Component - Clickable for filtering
...
function SidebarStatsCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left relative overflow-hidden rounded-2xl p-4 border-2 transition-all duration-300 hover:scale-[1.02] hover:shadow-lg ${
        isActive
          ? `${bgGradient} border-indigo-300 shadow-md`
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        ...
        {isActive && (
          <CaretRight weight="bold" size={16} className="text-indigo-500" />
        )}
      </div>
      <div className="relative h-1.5 bg-gray-200 rounded-full overflow-hidden">
        <div
          className={`absolute inset-y-0 left-0 bg-gradient-to-r ${gradient} rounded-full transition-all duration-500`}
          style={{ width: `${percentage}%` }}
        />
      </div>
      <p className="text-xs text-gray-400 mt-1">{percentage.toFixed(0)}% of total</p>
    </button>
  );
}
...
function QuickFilter({ label, count, isActive, onClick, colorClass }: QuickFilterProps) {
  return (
    <button
      onClick={onClick}
    ...
    >
      {label}
      <span className={`px-1.5 py-0.5 rounded-full text-xs ${isActive ? "bg-white/30" : "bg-white"}`}>
        {count}
      </span>
    </button>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 px-4">
      <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center mb-5">
        <Briefcase weight="duotone" size={40} className="text-indigo-500" />
      </div>
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="rounded-2xl p-4 bg-white/60">
          <div className="flex items-center gap-3 mb-3">
  ...
  return (
    <ThemeProvider theme={theme}>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30">
        ...
        <div className="relative max-w-[1600px] mx-auto px-4 sm:px-6 py-6">
          {/* Header */}
          <div className="mb-6">
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="lg:w-72 xl:w-80 flex-shrink-0">
              <div className="lg:sticky lg:top-6 space-y-4">
                {/* Stats Overview Card */}
                <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-gray-100 shadow-lg p-4">
                  <div className="flex items-center gap-2 mb-4">
                    <TrendUp weight="duotone" size={20} className="text-indigo-600" />
                  ...
                  {loading ? (
                    ...
                    <div className="space-y-3">
                      <SidebarStatsCard
                        icon={<Briefcase weight="fill" size={20} />}
                      ...
                      />
                      <SidebarStatsCard
                        icon={<Clock weight="fill" size={20} />}
                        label="Pending"
                        value={stats.pending}
                        total={stats.total}
                ...
                {!loading && platforms.length > 0 && (
                  <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-gray-100 shadow-lg p-4">
                    <div className="flex items-center gap-2 mb-3">
                      <Buildings weight="duotone" size={20} className="text-indigo-600" />
                      <h2 className="font-semibold text-gray-900 text-sm">Platforms</h2>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <QuickFilter
                        ...
                        colorClass="bg-indigo-100 text-indigo-700 ring-indigo-300"
                      />
                      {platforms.map((platform) => {
                        ...
                        return (
                          <QuickFilter
                            ...
                            label={platform}
                            count={count}
                            isActive={platformFilter.toLowerCase() === platform.toLowerCase()}
                            onClick={() =>
                              setPlatformFilter(
                            ...
                            colorClass={`${colors.bg} ${colors.text} ring-gray-300`}
                          />
                        );
                      })}
                    </div>
                ...
                {!loading && stats.total > 0 && (
                  <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl p-4 text-white shadow-lg">
                    <p className="text-sm text-indigo-100 mb-1">Success Rate</p>
                    <p className="text-3xl font-bold mb-2">
                      {((stats.applied / stats.total) * 100).toFixed(1)}%
                    </p>
                    <LinearProgress
                      variant="determinate"
                      value={(stats.applied / stats.total) * 100}
                      sx={{
                        height: 6,
                        borderRadius: 3,
                    ...
                    <p className="text-xs text-indigo-100 mt-2">
                      {stats.applied} successful out of {stats.total} applications
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-gray-100 shadow-lg p-4 mb-4">
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <div className="flex-1">
                    <TextField
                      fullWidth
                      size="small"
                      placeholder="Search jobs, companies, or applicants..."
                      value={searchQuery}
                ...
                {(searchQuery ||
                  ...
                  <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
                    <Funnel weight="duotone" size={14} className="text-gray-400" />
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {statusFilter !== "all" && (
                        <Chip
                          ...
                          className="bg-indigo-100 text-indigo-700 text-xs capitalize"
                        />
                      )}
                      {platformFilter !== "all" && (
                        <Chip
